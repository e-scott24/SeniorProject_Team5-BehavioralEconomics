    ...
    public class Round3TransportationModel : PageModel
    {
        ...
        public void OnGet(int playerId)
        {
            ...
            if (Player != null)
            {
                ...
                Cards = _gcService.GetCardsForRound(3, difficulty);
            }
        }

        // Returns JSON — JS shows the card overlay without a page reload
        public IActionResult OnPostSelectVehicle(int playerId, string vehicleDescription,
        ...
        public IActionResult OnPostApplyCard(int playerId, int cardId)
        {
            ...
            var cards = _gcService.GetCardsForRound(3, difficulty);
            var card = cards.FirstOrDefault(c => c.Id == cardId);

            if (card == null) return new JsonResult(new { success = false });

            _gameEngine.ApplyGameChanger(playerId, card, 3);
