    ...
    public class Round2LoansModel : PageModel
    {
        ...
        public void OnGet(int playerId)
        {
            ...
            if (Player != null)
            {
                ...
                Cards = _gcService.GetCardsForRound(2, difficulty);
            }
        }

        // Returns JSON — JS shows the card overlay without a page reload
        public IActionResult OnPostSelectLoan(int playerId, string loanDescription,
                                               decimal monthlyPayment, decimal totalDebt)
        {
            ...
            if (player != null) player.TotalDebt += totalDebt;
        ...
        public IActionResult OnPostApplyCard(int playerId, int cardId)
        {
            ...
            var cards = _gcService.GetCardsForRound(2, difficulty);
            var card = cards.FirstOrDefault(c => c.Id == cardId);

            if (card == null) return new JsonResult(new { success = false });

            _gameEngine.ApplyGameChanger(playerId, card, 2);
