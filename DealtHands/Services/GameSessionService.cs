...
namespace DealtHands.Services
{
    ...
    public class GameSessionService
    {
        ...
        /// Uses serializable transaction isolation to prevent race conditions on duplicate key constraint.
        /// </summary>
            ...
            await strategy.ExecuteAsync(async () =>
            {
                using (var transaction = await _context.Database.BeginTransactionAsync(IsolationLevel.Serializable))
                {
                    try
                    {
                        ...
                        lock (_randomLock)
                        {
                            randomIndex = _random.Next(availableCards.Count);
                        }
                        var assignedCard = availableCards[randomIndex];
        ...
        /// This method only adds Ugc entities to the change tracker. Caller is responsible for committing (SaveChangesAsync).
        /// </summary>
        private async Task AssignCardsToPlayersAsync(long gameRoundId, long gameSessionId, string roundType, List<long> userIds)
        {
            ...
            // NOTE: callers will call SaveChangesAsync when appropriate.
        }
        ...
        public async Task<Ugc?> AssignGameChangerAsync(long userId, long gameRoundId, long gameSessionId, string roundType)
        {
            ...
            var gameChanger = PickWeightedGameChanger(candidates);
            ...
            // Wrap update and add in a transaction to avoid partially applied state (user flags + UGC)
            var strategy = _context.Database.CreateExecutionStrategy();
            await strategy.ExecuteAsync(async () =>
            {
                using (var transaction = await _context.Database.BeginTransactionAsync())
        ...
        /// <summary>
        /// Picks a GameChanger from the candidate list using weighted random selection.
        /// </summary>
        private GameChanger PickWeightedGameChanger(List<GameChanger> candidates)
        {
            const int defaultWeight = 5;

            int totalWeight = 0;
            foreach (var c in candidates)
            {
                int w = c.Weight ?? defaultWeight;
                if (w < 1) w = 1;
                totalWeight += w;
            }
